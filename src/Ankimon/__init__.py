...
learnset_path = addon_dir / "modified_learnsets.json"
...
from .download_pokemon_data import create_pokedex, save_to_json, get_pokemon_data, get_pokemon_species_data, get_evolution_chain, fetch_pokemon_data
from .download_berries import berries_img
from .download_pokemon_sprites import download_front_back_sprites
...
def generate_random_pokemon():
    ...
    pokemon_encounter = 0
    generation_file = "generation_1.json"
    try:
        with open(str(addon_dir/generation_file), "r") as json_file:
            wild_pokemon_data = json.load(json_file)
            # Select 1 Pokémon from the list
            ...
            for random_pokemon_data in random_pokemon_list:
                ...
                if min_level < level:
                    ...
                    enemy_attacks = get_random_moves_for_pokemon(name, level)
                    base_experience = random_pokemon_data["base_experience"]
                    growth_rate = random_pokemon_data["growth_rate"]
                    description = random_pokemon_data["description"]
                    if gender is None:
                    ...
                    max_hp = hp
                    global ev_yield
                    ev_yield = random_pokemon_data["effort_values"]
                    return name, id, level, ability, type, stats, enemy_attacks, base_experience, growth_rate, hp, max_hp, description, ev, iv, gender, battle_status, battle_stats
                else:
...
def catch_pokemon(nickname):
    global pokemon_hp, name, ability, enemy_attacks, type, stats, base_experience, level, description, growth_rate, gender, id, iv
...
def new_pokemon():
    global name, id, level, hp, max_hp, ability, type, attacks, description, base_experience, stats, battlescene_file, ev, iv, gender, battle_status
    global font_file
    global font_name
    # new pokemon
    gender = None
    name, id, level, ability, type, stats, enemy_attacks, base_experience, growth_rate, hp, max_hp, description, ev, iv, gender, battle_status, battle_stats = generate_random_pokemon()
...
def search_pokeapi_db(pokemon_name,variable):
    global addon_dir
    pokeapi_db_path = addon_dir / "merged_file.json"
...
def download_pokemon_function():
    global addon_dir

    # Function to download Pokemon data (replace this with your actual code)
    def download_pokemon_data():
        ...
        chosen_generation = 1
        pokedex_data = create_pokedex(chosen_generation)
        save_to_json(pokedex_data, str(addon_dir / "generation_1.json"))
        # For example, you can use the requests library to make an API call

    # Function to update the loading window text
    def update_loading_text(window, layout):
        window.label.setText("Pokemon Data has been downloaded. You can now close this window")

    # Function to create and show the loading window
    def show_loading_window():
        # Create a QDialog window
        window = QDialog(mw)
        window.setWindowTitle("Loading Pokemon Data")

        # Add a QLabel for displaying text
        window.label = QLabel("Loading Pokemon Data... \n This may take several minutes", window)
        window.label.setAlignment(Qt.AlignCenter)

        # Add a QProgressBar for a spinning sign
        window.progress = QProgressBar(window)
        window.progress.setRange(0, 0)  # Set range to (0, 0) for a spinning sign
        ...
        # Start a new thread for downloading Pokemon data to prevent GUI freezing
        import threading
        thread = threading.Thread(target=download_pokemon_data)
        thread.start()

        # Function to update loading window text once the download is complete
        def update_text():
            thread.join()  # Wait for the download thread to finish
            update_loading_text(window, layout)

        # Use QTimer to periodically check if the download is complete
        timer = QTimer()
    ...
    # Call the function to show the loading window
    show_loading_window()
...
def download_sprites():
    ...
    id_to = 649 #pokeapi free to uses
...
def choose_pokemon(starter_name):
    ...
    name = search_pokedex(starter_name, "name")
    description = search_pokedex(starter_name, "types")
    generation_file = "merged_file.json"
...
qconnect(test_action3.triggered, download_pokemon_function)
