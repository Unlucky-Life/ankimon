...
battlescene_path_without_dialog = (
    addon_dir / "addon_sprites" / "battle_scenes_without_dialog"
)
...
pokenames_lang_path = (
    addon_dir / "user_files" / "data_files" / "pokemon_species_names.csv"
)
pokedesc_lang_path = (
    addon_dir / "user_files" / "data_files" / "pokemon_species_flavor_text.csv"
)
...
pokemon_species_legendary_path = (
    addon_dir / "user_files" / "pkmn_data" / "legendary.json"
)
...
if online_connectivity != False:
    ...
    class UpdateNotificationWindow(QDialog):
        def __init__(self, content):
            ...
            )  # For horizontal scrollbar, if you want it off
    ...
    github_url = (
        "https://raw.githubusercontent.com/Unlucky-Life/ankimon/main/update_txt.md"
    )
...
sound_list = [
    ...
    "crabrawler.mp3",
    "cradily.mp3",
    "cramorant-gorging.mp3",
    "cramorant-gulping.mp3",
    "cramorant.mp3",
    "cranidos.mp3",
    "crawdaunt.mp3",
    "cresselia.mp3",
    ...
    "natu.mp3",
    "naviathan.mp3",
    "necrozma-dawnwings.mp3",
    "necrozma-duskmane.mp3",
    "necrozma-ultra.mp3",
    "necrozma.mp3",
    "necturine.mp3",
    "necturna.mp3",
    ...
    "tapulele.mp3",
    "tarountula.mp3",
    "tatsugiri-droopy.mp3",
    "tatsugiri-stretchy.mp3",
    "tatsugiri.mp3",
    "tauros.mp3",
    "teddiursa.mp3",
    "tentacool.mp3",
    ...
    def generate_random_pokemon():
        ...
        try:
            ...
            if mainpokemon_level or mainpokemon_level != None:
                try:
                    level = random.randint(
                        (mainpokemon_level - (random.randint(0, var_level))),
                        (mainpokemon_level + (random.randint(0, var_level))),
                    )  # Random level between 1 and 100
            ...
            if (
                min_level is None
                or not min_level
                or mainpokemon_level is None
                or not mainpokemon_level
            ):
                level = 5
                min_level = 0
            if min_level < level:
...
def kill_pokemon():
    ...
    mainpokemon_level = save_main_pokemon_progress(
        mainpokemon_path,
        mainpokemon_level,
        mainpokemon_name,
...
def get_pokemon_by_category(category_name):
    ...
    pokemon_in_tier = [
        ...
        if pokemon["Tier"].lower() == category_name
    ]
    random_pokemon_name_from_tier = f"{(random.choice(pokemon_in_tier)).lower()}"
    random_pokemon_name_from_tier = special_pokemon_names_for_min_level(
        random_pokemon_name_from_tier
    )
    return (
        random_pokemon_name_from_tier  # return random pokemon name from that category
    )
...
def choose_random_pkmn_from_tier():
    ...
    try:
        if card_counter < (40 * cards_per_round):
            ...
            possible_tiers.extend(
                ["Baby", "Normal", "Baby", "Normal", "Normal", "Normal", "Normal"]
            )
        ...
        showWarning(
            f" An error occured with generating following Pkmn Info: {id}{pokemon_species} \n Please post this error message over the Report Bug Issue"
        )
        ...
        if mainpokemon_evolution:
                    ...
                    for mainpkmndata in main_pokemon_data:
                        if mainpkmndata["name"] == mainpokemon_name.capitalize():
                            ...
                            new_attacks = get_levelup_move_for_pokemon(
                                mainpokemon_name.lower(), int(mainpokemon_level)
                            )
                            if new_attacks:
                                msg = ""
                                ...
                                if len(attacks) < 4:
                                    ...
                                    if dialog.exec() == QDialog.DialogCode.Accepted:
                                        ...
                                        if index_to_replace is not None:
                                            ...
                                            showInfo(
                                                f"'{selected_attack}' not found in the list"
                                            )
                                    else:
            ...
            for mainpkmndata in main_pokemon_data:
                if mainpkmndata["name"] == mainpokemon_name.capitalize():
                    ...
                    if new_attacks:
                        showInfo(
                            f"Your {mainpokemon_name.capitalize()} can now learn a new attack !"
                        )
                    for new_attack in new_attacks:
                        if len(attacks) < 4:
                            attacks.append(new_attack)
                    ...
                    if pokemon_data["name"] == pkmn_name.capitalize():
                        ...
                        if pokemon is not None:
                            ...
                            if search_pokedex(evoName, "evos"):
                                pokemon["evos"].append(
                                    search_pokedex(evoName.lower(), "evos")
                                )
                            ...
                            pokemon["growth_rate"] = search_pokeapi_db_by_id(
                                evoId, "growth_rate"
                            )
                            pokemon["base_experience"] = search_pokeapi_db_by_id(
                                evoId, "base_experience"
                            )
                            ...
                            showInfo(
                                f"Your {pkmn_name.capitalize()} has evolved to {evoName.capitalize()}! \n You can now close this Window."
                            )
    except Exception as e:
        showWarning(f"{e}")
        ...
        with open(mainpokemon_path, "r") as json_file:
            ...
            for pokemon in main_pokemon_data:
                if pokemon["name"] == pkmn_name.capitalize():
                    attacks = pokemon["attacks"]
                    new_attacks = get_random_moves_for_pokemon(
                        pkmn_name.lower(), int(main_pokemon_data["level"])
                    )
                    for new_attack in new_attacks:
                        if len(attacks) < 4:
                            attacks.append(new_attack)
...
def catch_pokemon(nickname):
    ...
    if caught == 1:
        ...
        if pop_up_dialog_message_on_defeat is True:
            showInfo(
                "You have already caught the pokemon. Please close this window!"
...
def calc_atk_dmg(
    ...
    damage = (
        (((((2 * level * critical) + 2) / 5) * power * stat_atk / wild_stat_def) + 2)
        / 50
        * stab
        * eff
        * random_factor
    ...
    try:
        with open(str(mainpokemon_path), "r", encoding="utf-8") as json_file:
            ...
            for main_pokemon_data in main_pokemon_datalist:
                ...
                mainpokemon_hp = calculate_hp(
                    mainpokemon_hp_base_stat,
                    mainpokemon_level,
                    mainpokemon_ev,
                    mainpokemon_iv,
                )
                mainpokemon_current_hp = calculate_hp(
                    mainpokemon_hp_base_stat,
                    mainpokemon_level,
                    mainpokemon_ev,
                    mainpokemon_iv,
...
def get_effectiveness(move_type):
    ...
    attacking_types = [
        attacking_type.capitalize() for attacking_type in attacking_types
    ]
    defending_types = [
        defending_type.capitalize() for defending_type in defending_types
    ]
    with open(effectiveness_chart_file_path, "r", encoding="utf-8") as json_file:
        ...
        for attacking_type in attacking_types:
            if attacking_type in data:
                ...
                eff_values = [
                    data[attacking_type][defending_type]
                    for defending_type in defending_types
                ]
                effectiveness_values.extend(
                    eff_values
                )  # Use extend to add values to the list
        ...
        if reviewed_cards_count >= cards_per_round:
            ...
            if pokemon_encounter > 0 and hp > 0:
                ...
                if battle_status != "fighting":
                    msg, acc, battle_status, stats = status_effect(
                        battle_status,
                        name,
                        move,
                        hp,
                    ...
                    if category == "Status":
                        ...
                        if move["basePower"] == 0:
                            ...
                            if secondary is not None:
                                bat_status = move.get("secondary", None).get(
                                    "status", None
                                )
...
def effect_status_moves(
    move_name, mainpokemon_stats, stats, msg, name, mainpokemon_name
):
        ...
        with open(mypokemon_path, "r") as json_file:
            ...
            if captured_pokemon_data:
                ...
                for pokemon in captured_pokemon_data:
                    ...
                    pokemon_description = search_pokeapi_db_by_id(
                    ...
                    if len(pokemon_type) > 1:
                        pokemon_button.clicked.connect(
                            lambda state, name=pokemon_name, level=pokemon_level, id=pokemon_id, ability=pokemon_ability, type=[
                                pokemon_type[0],
                                pokemon_type[1],
                            ], detail_stats=pokemon_stats, attacks=pokemon_attacks, base_experience=pokemon_base_experience, growth_rate=pokemon_growth_rate, description=pokemon_description, gender=pokemon_gender, nickname=pokemon_nickname: PokemonCollectionDetails(
                                name,
                                level,
                                id,
                                ability,
                    ...
                    choose_pokemon_button.clicked.connect(
                        lambda state, name=pokemon_name, level=pokemon_level, id=pokemon_id, ability=pokemon_ability, type=pokemon_type, detail_stats=pokemon_stats, attacks=pokemon_attacks, hp=pokemon_hp, base_experience=mainpokemon_base_experience, growth_rate=pokemon_growth_rate, ev=pokemon_ev, iv=pokemon_iv, gender=pokemon_gender: MainPokemon(
                            name,
                            level,
                            id,
                            ability,
...
def PokemonCollectionDetails(
    ...
    try:
        ...
        remember_attacks_details_button = QPushButton(
            "Remember Attacks"
        )  # add Details to Moves
        all_attacks = get_all_pokemon_moves(name, level)
        qconnect(
            remember_attacks_details_button.clicked,
            lambda: remember_attack_details_window(id, attacks, all_attacks),
        )
        ...
        qconnect(
            ...
            lambda: PokemonTrade(name, id, level, ability, iv, ev, gender, attacks),
        )
...
def remember_attack_details_window(id, attack_set, all_attacks):
    ...
    for attack in all_attacks:
        move = find_details_move(attack)
        remember_attack_button = QPushButton(
            f"Remember {attack}"
        )  # add Details to Moves
        remember_attack_button.clicked.connect(
            lambda checked, a=attack: remember_attack(id, attack_set, a)
        )
...
def PokemonTrade(name, id, level, ability, iv, ev, gender, attacks):
    ...
    if not found:
        ...
        qconnect(
            trade_button.clicked, lambda: PokemonTradeIn(trade_code_input.text(), name)
        )
        # Information label
        ...
        for row in csv_reader:
            if row[fieldnames[0]] == str(
                ...
                if pop_up_dialog_message_on_defeat is True:
                    showInfo(
                        (
                            f"Your main Pokemon {mainpokemon_name} Lvl {level} needs {experience} XP to reach the next level."
                        )
                    )
                break
    ...
    def download_pokemon_data(self):
        try:
            ...
            for i, url in enumerate(self.urls, start=1):
                if url != "POKEAPI":
                    ...
                    if response.status_code == 200:
                        ...
                        )  # Replace with a signal if needed
                ...
                for sprite_type in ["front_default", "back_default"]:
                    if sprite_type == "front_default":
                        ...
                        if response.status_code == 200:
                            ...
                            with open(
                                os.path.join(save_dir, f"{pokemon_id}.png"), "wb"
                            ) as f:
            ...
            for item_name in self.item_names:
                ...
                if response.status_code == 200:
                    with open(
                        os.path.join(self.items_destination_to, item_name), "wb"
                    ) as file:
            ...
            for badge_num in range(1, 68):
                ...
                if response.status_code == 200:
                    with open(
                        os.path.join(self.badges_destination_to, badge_file), "wb"
                    ) as file:
                        file.write(response.content)
            ...
            for sound in self.sound_names:
                ...
                if response.status_code == 200:
                    with open(
                        os.path.join(self.sounds_destination_to, sound), "wb"
                    ) as file:
                        file.write(response.content)
            ...
            for pokemon_id in range(1, self.id_to + 1):
                for sprite_type in ["front_showdown", "back_showdown"]:
                    ...
                    if response.status_code == 200:
                        with open(
                            os.path.join(self.front_gif_dir, f"{pokemon_id}.gif"), "wb"
                        ) as file:
                            file.write(response.content)
                    ...
                    if response.status_code == 200:
                        with open(
                            os.path.join(self.back_gif_dir, f"{pokemon_id}.gif"), "wb"
                        ) as file:
                            file.write(response.content)
                    progress = int((pokemon_id / self.id_to) * 100)
                    self.progress_updated.emit(progress)
...
class LoadingDialog(QDialog):
    ...
    def start_download(self, addon_dir):
        ...
        self.downloader.downloading_item_sprites_txt.connect(
            self.downloading_item_sprites_txt
        )
        self.downloader.downloading_badges_sprites_txt.connect(
            self.downloading_badges_sprites_txt
        )
        self.downloader.downloading_sounds_txt.connect(self.downloading_sounds_txt)
        self.downloader.downloading_gif_sprites_txt.connect(
            self.downloading_gif_sprites_txt
        )
        self.downloader.progress_updated.connect(self.progress.setValue)
...
if database_complete != False:
    ...
    def inject_life_bar(web_content, context):
        ...
        if reviewer_image_gif == False:
            pokemon_imagefile = (
                f'{search_pokedex(name.lower(), "num")}.png'  # use for png files
            )
            pokemon_image_file = os.path.join(
                frontdefault, pokemon_imagefile
            )  # use for png files
            if show_mainpkmn_in_reviewer is True:
                main_pkmn_imagefile = f"{mainpokemon_id}.png"  # use for png files
                main_pkmn_imagefile_path = os.path.join(
                    backdefault, main_pkmn_imagefile
                )  # use for png files
        else:
            pokemon_imagefile = f'{search_pokedex(name.lower(), "num")}.gif'
            pokemon_image_file = os.path.join(
                (user_path_sprites / "front_default_gif"), pokemon_imagefile
            )
            if show_mainpkmn_in_reviewer is True:
                main_pkmn_imagefile = f"{mainpokemon_id}.gif"
                main_pkmn_imagefile_path = os.path.join(
                    (user_path_sprites / "back_default_gif"), main_pkmn_imagefile
                )
        if show_mainpkmn_in_reviewer is True:
            mainpkmn_max_hp = calculate_hp(
                mainpokemon_stats["hp"],
                mainpokemon_level,
                mainpokemon_ev,
                mainpokemon_iv,
            )
        ...
        if not life_bar_injected and is_reviewer:
            ...
            web_content.body += (
                f'<div id="name-display">{name.capitalize()} LvL: {level}</div>'
            )
            if hp > 0:
    ...
    def update_life_bar(reviewer, card, ease):
        ...
        reviewer.web.eval(
            'document.getElementById("life-bar").style.width = "'
            + str(pokemon_hp_percent)
            + '%";'
        )
        reviewer.web.eval(
            'document.getElementById("life-bar").style.background = "linear-gradient(to right, '
            + str(hp_color)
            + ", "
            + str(hp_color)
            + " "
        ...
        reviewer.web.eval(
            'document.getElementById("life-bar").style.boxShadow = "0 0 10px rgba('
            + str(hp_color)
            + ')";'
        )
        ...
        new_html_content = (
            f'<img src="data:image/png;base64,{image_base64}" alt="PokeImage">'
        )
        reviewer.web.eval(
            f'document.getElementById("PokeImage").innerHTML = `{new_html_content}`;'
        )
        reviewer.web.eval(
            f'document.getElementById("pokestatus").innerHTML = `{status_html}`;'
        )
        if show_mainpkmn_in_reviewer is True:
            new_html_content_mainpkmn = f'<img src="data:image/png;base64,{image_base64_mainpkmn}" alt="MyPokeImage">'
            main_name_display_text = (
                f"{mainpokemon_name.capitalize()} Lvl: {mainpokemon_level}"
            )
            main_hp_display_text = f"HP: {mainpokemon_hp}/{mainpkmn_max_hp}"
            reviewer.web.eval(
                'document.getElementById("mylife-bar").style.width = "'
                + str(mainpkmn_hp_percent)
                + '%";'
            )
            reviewer.web.eval(
                'document.getElementById("mylife-bar").style.background = "linear-gradient(to right, '
                + str(myhp_color)
                + ", "
                + str(myhp_color)
                + " "
            ...
            reviewer.web.eval(
                'document.getElementById("mylife-bar").style.boxShadow = "0 0 10px rgba('
                + str(myhp_color)
                + ')";'
            )
            reviewer.web.eval(
                f'document.getElementById("MyPokeImage").innerHTML = `{new_html_content_mainpkmn}`;'
            )
            reviewer.web.eval(
                'document.getElementById("myname-display").innerText = "'
                + main_name_display_text
                + '";'
            )
            reviewer.web.eval(
                'document.getElementById("myhp-display").innerText = "'
                + main_hp_display_text
                + '";'
            )
...
class TestWindow(QWidget):
    ...
    def pokemon_display_first_encounter(self):
        ...
        def window_show():
            ...
            experience = find_experience_for_level(
                mainpokemon_growth_rate, mainpokemon_level
            )
            experience = int(experience)
            mainxp_bar_width = 5
...
class EvoWindow(QWidget):
    ...
    def pokemon_display_evo(self, pkmn_name, prevo_name):
        ...
        custom_font = load_custom_font(20)
        message_box_text = (
            f"{(prevo_name).capitalize()} has evolved to {(pkmn_name).capitalize()} !"
        )
        # Draw the text on top of the image
        # Adjust the font size as needed
        painter.setFont(custom_font)
    ...
    def display_pokemon_evo(self, pkmn_name):
        ...
        pokemon_images, evolve_button, dont_evolve_button = (
            self.pokemon_display_evo_pokemon(pkmn_name)
        )
    ...
    def pokemon_display_evo_pokemon(self, pkmn_name):
        ...
        window_title = (
            f"{pkmn_name.capitalize()} is evolving to {pokemon_evo.capitalize()} ?"
        )
        # Display the Pokémon image
...
class ItemWindow(QWidget):
    ...
    def ItemLabel(self, item_name):
        ...
        item_picture_label.setStyleSheet(
        ...
        use_item_button.clicked.connect(
            lambda: self.Check_Evo_Item(comboBox.currentText(), item_name)
        )
    ...
    def Check_Evo_Item(self, pkmn_name, item_name):
        try:
            ...
            if (
                evoItem == item_name
            ):  # Corrected this line to assign the item_name to evoItem
