...
class Reviewer_Manager:
    ...
    def inject_life_bar(self, web_content, context):
        if int(self.settings.get("gui.show_mainpkmn_in_reviewer", 1)) < 3:
            ...
            # Reduce the width of both HP bars to avoid overlap
            hp_bar_width_percent = 40  # Make both bars 40% width instead of 50%/100%
            if int(self.settings.get('gui.show_mainpkmn_in_reviewer', 1)) > 0:
                pokemon_hp_percent = int((self.enemy_pokemon.hp / self.enemy_pokemon.max_hp) * hp_bar_width_percent)
                main_pkmn_hp_percent = int((self.main_pokemon.hp / self.main_pokemon.max_hp) * hp_bar_width_percent)
            else:
                pokemon_hp_percent = int((self.enemy_pokemon.hp / self.enemy_pokemon.max_hp) * 80)  # Also reduce if only one bar
            ...
            if not self.life_bar_injected and is_reviewer:
                css = create_css_for_reviewer(
                    ...
                    main_pkmn_hp_percent if int(self.settings.get('gui.show_mainpkmn_in_reviewer', 1)) > 0 else 0,
                ...
                .night_mode #myhp-display, .night_mode #xp_text {
                    background: #2C2C2C !important;
                    color: white !important;
                    border-radius: 5px !important;
                    padding: 4px 8px !important;
                }
                .night_mode #life-bar, .night_mode #mylife-bar, .night_mode #xp-bar {
                    border: 2px solid #2C2C2C !important;
                }
                """
    ...
    def update_life_bar(self, reviewer, card, ease):
        if int(self.settings.get("gui.show_mainpkmn_in_reviewer", 1)) < 3:
            ...
            if self.settings.get('gui.reviewer_image_gif', 1) == False:
                ...
                if int(self.settings.get('gui.show_mainpkmn_in_reviewer', 1)) > 0:
                    if self.settings.compute_special_variable('view_main_front') == -1:
                        ...
                        side = "back"
                    main_pkmn_imagefile_path = self.main_pokemon.get_sprite_path(side, "gif")
            # Use the same reduced width for both HP bars
            hp_bar_width_percent = 40
            if int(self.settings.get('gui.show_mainpkmn_in_reviewer', 1)) > 0:
                pokemon_hp_percent = int((self.enemy_pokemon.hp / self.enemy_pokemon.max_hp) * hp_bar_width_percent)
                image_base64_mainpkmn = get_image_as_base64(main_pkmn_imagefile_path)
                main_pkmn_hp_percent = int((self.main_pokemon.hp / self.main_pokemon.max_hp) * hp_bar_width_percent)
            else:    
                pokemon_hp_percent = int((self.enemy_pokemon.hp / self.enemy_pokemon.max_hp) * 80)
            image_base64 = get_image_as_base64(pokemon_image_file)
            ...
            if self.settings.get("gui.styling_in_reviewer", True) is True:
                ...
                reviewer.web.eval('document.getElementById("life-bar").style.background = "' + hp_color + '";')
                reviewer.web.eval('document.getElementById("life-bar").style.boxShadow = "none";')
                reviewer.web.eval('document.getElementById("life-bar").style.border = window.matchMedia("(prefers-color-scheme: dark)").matches ? "2px solid #2C2C2C" : "2px solid white";')
                if self.settings.get("xp_bar_config", False) is True:
                    ...
                    reviewer.web.eval('document.getElementById("xp-bar").style.boxShadow = "none";')
                    reviewer.web.eval('document.getElementById("xp-bar").style.border = window.matchMedia("(prefers-color-scheme: dark)").matches ? "2px solid #2C2C2C" : "2px solid white";')
                enemy_lang_name = (get_pokemon_diff_lang_name(int(self.enemy_pokemon.id), int(self.settings.get('misc.language'))).capitalize())
                if self.enemy_pokemon.shiny is True:
                    enemy_lang_name += " ⭐ "
                name_display_text = f"{enemy_lang_name} LvL: {self.enemy_pokemon.level}"
                ...
                if int(self.settings.get('gui.show_mainpkmn_in_reviewer', 1)) > 0:
                    ...
                    reviewer.web.eval('document.getElementById("mylife-bar").style.width = "' + str(main_pkmn_hp_percent) + '%";')
                    reviewer.web.eval('document.getElementById("mylife-bar").style.background = "' + str(myhp_color) + '";')
                    reviewer.web.eval('document.getElementById("mylife-bar").style.boxShadow = "none";')
                    reviewer.web.eval('document.getElementById("mylife-bar").style.border = window.matchMedia("(prefers-color-scheme: dark)").matches ? "2px solid #2C2C2C" : "2px solid white";')
